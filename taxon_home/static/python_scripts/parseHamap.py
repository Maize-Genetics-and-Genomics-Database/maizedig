...
try:
    hamapFile = open('/var/www/BioDIG/taxon_home/static/python_scripts/hamap_families.dat','r')
except IOError:
...
'''
try:
    delimitedFile = open('/var/www/BioDIG/taxon_home/static/python_scripts/ontologies.txt', 'w')
except IOError:
...
for key in hashOntology.keys():
    ...
    conn = psycopg2.connect(database = "chado", user = "ktcho", password = "dev_biodiG", host = "localhost")
    cur = conn.cursor()
    ...
    conn2 = psycopg2.connect(database = "chado", user = "ktcho", password = "dev_biodiG", host = "localhost")
    cur2 = conn2.cursor()
