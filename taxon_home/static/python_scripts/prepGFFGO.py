...
try:
    ...
    connDesc = MySQLdb.connect(host="localhost", user="ktcho", passwd="dev_biodiG", db="go")
...
'''
try:  
    delimitedFile = open('/var/www/BioDIG/taxon_home/static/python_scripts/color_distribution.txt', 'w')
except IOError:
...
for line in gffFile:
    ...
    if (len(splitLine) > 2 and splitLine[2] == 'gene'):
        ...
        try:   
            connMySQL = MySQLdb.connect(host="localhost", user = "ktcho",passwd = "deb_biodiG",db = "go")
